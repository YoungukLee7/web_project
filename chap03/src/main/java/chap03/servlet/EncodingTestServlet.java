 ...
 */
@WebServlet(
		description = "한글 인코딩 테스트를 해주는 서블릿",
...
public class EncodingTestServlet extends HttpServlet {
	...
	/*
	  ...
	  URL의 해것은 Tomcat이 담당하기 때문에 URL 뒤에 추가된 데이터를
	  올바른 인코딩 타입으로 해석하기 위해서는 server.xml의 설정을 변경해야 한다
	  (포트번호 설정하는 곳에  URIEncoding="EUC-KR" 추가)
	  (말 그대로 GET방식은 URL에 데이터가 딸려오기 때문에 URL전체를 인코딩하여 한글이 나오게 하려면 서버에 추가해야한다)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// ContextPath : 우리 프로젝트의 최상의 URL (add and remove 할때 server.xml에 자동으로 추가된다)
